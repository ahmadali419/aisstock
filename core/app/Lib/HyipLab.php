...
class HyipLab
{
    ...
    public static function levelCommission($user, $amount, $commissionType, $trx, $setting)
    {
        ...
        // Loop through the referral levels
        while ($i <= $level) {
            $me = $meUser;
            $refer = $me->referrer;
    
            // Break if there is no referrer
            if (empty($refer)) {
                break;
            }
            ...
            // Break if no commission record is found for the current level
            if (!$commission) {
                break;
            }
            ...
            // Determine the commission type for notification
            $comType = match($commissionType) {
                'deposit_commission' => 'Deposit',
                'interest_commission' => 'Interest',
                default => 'Invest',
            };
        ...
        // Insert all transactions at once to optimize performance
