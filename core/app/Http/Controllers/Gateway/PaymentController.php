    ...
    public function depositTransaction(Request $request){
        try {
            ...
            $request->validate([
                'amount' => 'required|numeric|gt:0',
                'txid' => 'required',
                'oneTimeKey' => ['required', new ValidOneTimeKey],
                'file' => 'required|file|mimes:jpg,jpeg,png|max:2048', // Adjust 'mimes' and 'max' as needed
            ]);
            ...
            if ($request->hasFile('file')) {
                ...
                // Prepare file details to be stored in the details column
                $fileDetails[] = [
                    'name' => 'screenshot',
                    'type' => 'file ',
                    'value' => $value,
            ...
            // Create a new Deposit instance and save the data
            $data = new Deposit();
            $user = auth()->user();
            $final_amo = $request->amount / gs('stock_current_rate');
    
            $data->user_id = $user->id;
            $data->method_code = 1000;
            $data->method_currency = 'USTD';
            $data->amount = $request->amount;
            $data->charge = 0;
            $data->rate = gs('stock_current_rate');
            $data->final_amo = $final_amo;
            $data->btc_amo = 0;
            $data->status = 2;
            $data->btc_wallet = "";
            $data->trx = $request->txid;
            $data->detail =$fileDetails;
            $data->save();
            ...
            $notify[] = ['success', 'Your Amount has been deposited successfully'];
            return back()->withNotify($notify);
        } catch (\Exception $e) {
